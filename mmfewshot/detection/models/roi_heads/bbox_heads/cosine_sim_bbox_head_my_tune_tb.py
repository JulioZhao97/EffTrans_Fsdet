...
class CosineSimBBoxHeadMyTuneTB(ConvFCBBoxHeadMyTune):
    ...
    def __init__(self,
        ...
        #f = open('/data/zhaozhiyuan/mmfewshot-main/base_statistics/2022-03-16/17:13:15/base_statistics.pt', 'rb')
    ...
    def transform(self, embed, label):
        if label >= 10 and label <= 14:
            # select topk classes
            dist_all = [None for i in range(10)]
            for c, statistic in self.base_statistics.items():
                dist = F.pairwise_distance(embed.unsqueeze(0), torch.Tensor(statistic['mean']).to(embed.device).unsqueeze(0))
                dist_all[int(c)] = dist[0]
            dist_all = torch.Tensor(dist_all)
