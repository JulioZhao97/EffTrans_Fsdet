...
class CosineSimBBoxHeadMyTuneCOCOTestEMA(ConvFCBBoxHeadMyTune):
    ...
    def __init__(self,
        ...
        f = open('/data/zhaozhiyuan/mmfewshot-main/base_statistics/2022-03-04/09:27:01/base_statistics.pt', 'rb')
        ...
        self.COCO_ALL_CLASSES = sorted(self.COCO_BASE_CLASSES + self.COCO_NOVEL_CLASSES)
        self.COCO_IDMAP = {v: i for i, v in enumerate(self.COCO_ALL_CLASSES)}
        print(self.COCO_IDMAP)
        self.COCO_IDMAP_REV = dict(map(reversed, self.COCO_IDMAP.items()))
        print(self.COCO_IDMAP_REV)
    ...
    def transform(self, embed, label):
        # determine label
        COCOClass = self.COCO_IDMAP_REV[label.item()]
        
        if COCOClass in self.COCO_NOVEL_CLASSES:
            c = self.COCO_NOVEL_CLASSES.index(COCOClass)
            # select topk classes
            dist_all = [None for i in range(60)]
            for c, statistic in self.base_statistics.items():
                dist = F.pairwise_distance(embed.unsqueeze(0), torch.Tensor(statistic['mean']).to(embed.device).unsqueeze(0))
                dist_all[int(c)] = dist[0]
            dist_all = torch.Tensor(dist_all)
            ...
            if len(new_samples_all) > 0:
                ...
                new_samples_all = None
        elif COCOClass in self.COCO_BASE_CLASSES:
            c = self.COCO_BASE_CLASSES.index(COCOClass)
            
            # update novel mean&var using ema
            vt = self.base_statistics[str(c)]['mean']
            self.base_statistics[str(c)]['mean'] = self.beta * vt + (1 - self.beta) * embed.cpu().numpy()
