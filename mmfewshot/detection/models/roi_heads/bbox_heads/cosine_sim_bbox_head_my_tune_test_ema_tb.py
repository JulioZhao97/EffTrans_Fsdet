...
class CosineSimBBoxHeadMyTuneTestEMATB(ConvFCBBoxHeadMyTune):
    ...
    def __init__(self,
        ...
        f = open('/data/zhaozhiyuan/mmfewshot-main/base_statistics/tb/split2/base_statistics.pt', 'rb')
        ...
        self.novel_classes = 5
        self.dim = 1024
        self.mean_all_novel = [None for i in range(self.novel_classes)]
    ...
    def transform(self, embed, label):
        if label >= self.base_classes and label <= self.base_classes + self.novel_classes - 1:
            # update novel mean&var using ema
            if self.mean_all_novel[label - self.base_classes] is None:
                self.mean_all_novel[label - self.base_classes] = embed
            #else:
            #    vt = self.mean_all_novel[label - self.base_classes]
            #    self.mean_all_novel[label - self.base_classes] = self.beta_novel * vt + (1 - self.beta_novel) * embed
            
            # select topk classes
            dist_all = [None for i in range(self.base_classes)]
            for c, statistic in self.base_statistics.items():
                dist = F.pairwise_distance(embed.unsqueeze(0), torch.Tensor(statistic['mean']).to(embed.device).unsqueeze(0))
                dist_all[int(c)] = dist[0]
            dist_all = torch.Tensor(dist_all)
            ...
            for c in topk:
                ...
                variance += self.base_statistics[str(c.item())]['variance']
            cal_mean = (mean + embed.cpu().numpy()) / (self.k + 1)
            #cal_mean = (mean + self.mean_all_novel[label - self.base_classes].cpu().numpy()) / (self.k + 1)
            cal_var = variance / self.k + self.alpha
            ...
            # update base mean&var using ema
            vt = self.base_statistics[str(label.item())]['mean']
            self.base_statistics[str(label.item())]['mean'] = self.beta * vt + (1 - self.beta) * embed.cpu().numpy()
    ...
    def transform(self, embed, label):
        if label >= self.base_classes and label <= self.base_classes + self.novel_classes - 1:
            ...
            new_samples_all = []
            for i in range(self.new_feat_num):
                new_sample = m.sample((100,))
                # L2-distance
                d = torch.nn.functional.pairwise_distance(new_sample.cuda(), embed.unsqueeze(0))
                index = torch.linspace(0,99,steps=100)[d<self.min_dist]
                if index.shape[0] != 0:
                    new_sample = new_sample[index[0].long()]
                    new_samples_all.append(new_sample.unsqueeze(0))
            if len(new_samples_all) > 0:
                new_samples_all = torch.cat(new_samples_all, dim=0).to(embed.device)
            else:
                new_samples_all = None
            ...
            # update novel mean&var using ema
            vt = self.base_statistics[str(label.item())]['mean']
            self.base_statistics[str(label.item())]['mean'] = self.beta_base * vt + (1 - self.beta_base) * embed.cpu().numpy()
    ...
    def tukey_ladder(self, x, labels):
        for i, label in enumerate(labels):
            if i >= self.base_classes and i <= self.base_classes + self.novel_classes - 1:
                x[i] = x[i] ** self.phi
        return x
    ...
    def forward(self, x: Tensor, labels = None) -> Tuple[Tensor, Tensor]:
        ...
        if self.training and labels is not None:
            ...
            if len(new_sample_index) != 0:
                ...
                # no need to normalize fc weight
