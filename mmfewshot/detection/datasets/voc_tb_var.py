...
VOC_SPLIT_TB_VAR = dict(
    ALL_CLASSES_SPLIT1=('001-CCTV1', '011-history_channel'),
    NOVEL_CLASSES_SPLIT1=(),
    BASE_CLASSES_SPLIT1=('001-CCTV1', '011-history_channel'),
    ALL_CLASSES_SPLIT2=('002-beijingweishi', '004-CNN'),
    NOVEL_CLASSES_SPLIT2=(),
    BASE_CLASSES_SPLIT2=('002-beijingweishi', '004-CNN'),
    ALL_CLASSES_SPLIT3=('005-shandongweishi', '006-henanweishi'),
    NOVEL_CLASSES_SPLIT3=(),
    BASE_CLASSES_SPLIT3=('005-shandongweishi', '006-henanweishi'),
    ALL_CLASSES_SPLIT4=('010-sichuanweishi', '014-VOA'),
    NOVEL_CLASSES_SPLIT4=(),
    BASE_CLASSES_SPLIT4=('010-sichuanweishi', '014-VOA'),
    ALL_CLASSES_SPLIT5=('016-CBS', '017-KTN'),
    NOVEL_CLASSES_SPLIT5=(),
    BASE_CLASSES_SPLIT5=('016-CBS', '017-KTN'))

@DATASETS.register_module()
class FewShotVOCDatasetTBVAR(BaseFewShotDataset):
...
@DATASETS.register_module()
class FewShotVOCCopyDatasetTBVAR(FewShotVOCDatasetTBVAR):
...
@DATASETS.register_module()
class FewShotVOCDefaultDatasetTBVAR(FewShotVOCDatasetTBVAR):
    ...
    voc_benchmark = {
        f'SPLIT{split}_{shot}SHOT_TASK{task}': [
            dict(
                type='ann_file',
                ann_file=f'data/few_shot_ann/tb_var/benchmark_split{split}_{shot}shot/task{task}/'
                f'box_{shot}shot_{class_name}_train.txt',
                ann_classes=[class_name])
            for class_name in VOC_SPLIT_TB_VAR[f'ALL_CLASSES_SPLIT{split}']
        ]
        for shot in [1] for split in [1,2,3,4,5] for task in [0, 1, 2, 3, 4]
    }
    ...
    def ann_cfg_parser(self, ann_cfg: List[Dict]) -> List[Dict]:
        ...
        #print(ann_cfg)
        #print(new_ann_cfg)
        return super(FewShotVOCDatasetTBVAR, self).ann_cfg_parser(new_ann_cfg)
