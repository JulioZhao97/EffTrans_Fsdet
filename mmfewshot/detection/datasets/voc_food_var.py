...
VOC_SPLIT_FOOD_VAR = dict(
    ALL_CLASSES_SPLIT1=('7up', 'BudLight'),
    NOVEL_CLASSES_SPLIT1=(),
    BASE_CLASSES_SPLIT1=('7up', 'BudLight'),
    ALL_CLASSES_SPLIT2=('fanta', 'Mirinda'),
    NOVEL_CLASSES_SPLIT2=(),
    BASE_CLASSES_SPLIT2=('fanta', 'Mirinda'),
    ALL_CLASSES_SPLIT3=('Pizza Hut', 'Mountain Dew'),
    NOVEL_CLASSES_SPLIT3=(),
    BASE_CLASSES_SPLIT3=('Pizza Hut', 'Mountain Dew'),
    ALL_CLASSES_SPLIT4=('kfc', 'sprite'),
    NOVEL_CLASSES_SPLIT4=(),
    BASE_CLASSES_SPLIT4=('kfc', 'sprite'),
    ALL_CLASSES_SPLIT5=("wendy's", 'Dr Pepper'),
    NOVEL_CLASSES_SPLIT5=(),
    BASE_CLASSES_SPLIT5=("wendy's", 'Dr Pepper'))

@DATASETS.register_module()
class FewShotVOCDatasetFOODVAR(BaseFewShotDataset):
...
@DATASETS.register_module()
class FewShotVOCCopyDatasetFOODVAR(FewShotVOCDatasetFOODVAR):
...
@DATASETS.register_module()
class FewShotVOCDefaultDatasetFOODVAR(FewShotVOCDatasetFOODVAR):
    ...
    voc_benchmark = {
        f'SPLIT{split}_{shot}SHOT_TASK{task}': [
            dict(
                type='ann_file',
                ann_file=f'data/few_shot_ann/Flickr-FoodLogos-10/benchmark_split{split}_{shot}shot/task{task}/'
                f'box_{shot}shot_{class_name}_train.txt',
                ann_classes=[class_name])
            for class_name in VOC_SPLIT_FOOD_VAR[f'ALL_CLASSES_SPLIT{split}']
        ]
        for shot in [1] for split in [1,2,3,4,5] for task in [0]
    }
    ...
    def ann_cfg_parser(self, ann_cfg: List[Dict]) -> List[Dict]:
        ...
        #print(ann_cfg)
        #print(new_ann_cfg)
        return super(FewShotVOCDatasetFOODVAR, self).ann_cfg_parser(new_ann_cfg)
